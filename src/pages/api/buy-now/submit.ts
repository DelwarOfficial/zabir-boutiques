...
export async function POST(context: APIContext): Promise<Response> {
  ...
  // N-28: validated here, but only *required* once we know the order is
  // prepaid — see the resolvedPaymentMethod gate below, since a COD order can
  // still be escalated to partial_prepay server-side.
  ...
  // No coupon/discount on Buy Now, so subtotal is the taxable base directly.
  const vatRatePercent = await getVatRatePercent(env.DB, 'goods', now);
  const vatPaisa = assertPaisa(calculateVatPaisaCanonical(subtotalPaisa, vatRatePercent), 'vat_paisa');
  ...
  if (resolvedPaymentMethod !== 'cod' && !emailResult.ok) {
  ...
  try {
    ...
    // §11.7 step 5: largest-remainder per-line VAT (invariant: sum equals orders.vat_paisa).
    ...
    if (advancePaisa > 0) {
      try {
        ...
        // N-28: local payment row written before the provider call, so a
        // successful charge can never end up with nothing to reconcile
        // against. Same ordering as checkout.ts.
        await env.DB.prepare(
        ...
        const checkout = await createPaymentCheckout(env, {
          ...
          // N-28: callback URLs are built only from the trusted configured
          // site origin. The Origin header is caller-controlled and using it
          // here was an open redirect for a paying customer (same fix as K-13
          // in checkout.ts).
          redirectUrl: `${env.PUBLIC_SITE_URL}/api/payments/callback`,
          webhookUrl: `${env.PUBLIC_SITE_URL}/api/payments/webhook`,
          cancelUrl: `${env.PUBLIC_SITE_URL}/buy-now/${sessionId}`,
        });
