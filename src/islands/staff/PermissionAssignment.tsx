...
export function PermissionAssignment({ selected, onChange }: Props) {
  ...
  return (
    <div className="space-y-2 max-h-[420px] overflow-y-auto rounded-lg border border-line">
        ...
        return (
          <div key={group.category} className="border-b border-line last:border-b-0">
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm font-semibold text-ink hover:bg-surface-soft transition-colors"
            >
              <span className={`shrink-0 text-[10px] text-muted transition-transform ${isOpen ? 'rotate-90' : ''}`}>▶</span>
              <CategoryIcon id={group.category} />
              <span className="flex-1">{group.label}</span>
              <span className="text-[11px] text-muted tabular-nums">{selCount}/{total}</span>
            </button>
            {isOpen && (
              <div className="px-3 pb-2 space-y-1">
                <label className="flex items-center gap-2 py-1 text-[12px] font-medium text-brand cursor-pointer hover:opacity-80">
                  <input
                    type="checkbox"
                    checked={allSel}
                ...
                {group.permissions.map(perm => (
                  <label
                    key={perm.name}
                    className="flex items-start gap-2 py-1 px-2 rounded hover:bg-surface-soft cursor-pointer group"
                  >
                    <input
                      type="checkbox"
                    ...
                    <div className="min-w-0 flex-1">
                      <span className="text-sm font-medium text-ink">{perm.label}</span>
                      <p className="text-[11px] text-muted leading-tight mt-0.5">{perm.description}</p>
                      <code className="text-[10px] text-muted font-mono opacity-60 group-hover:opacity-100">{perm.name}</code>
                    </div>
                  </label>
                ))}
              </div>
