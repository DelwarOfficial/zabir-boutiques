    ...
    return (
      <div className="max-w-lg mx-auto text-center py-8 fade-up">
        <div className="w-14 h-14 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-4">
          <svg className="w-7 h-7 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M20 6 9 17l-5-5"/></svg>
        </div>
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          <a href={`/staff/products/${result.productId}`} className="inline-flex items-center gap-1.5 px-4 py-2 bg-brand text-white text-sm font-semibold rounded-lg no-underline hover:opacity-90 transition-all">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            View Product
          </a>
    ...
    <div className="max-w-3xl">
      {error && (
        <div className="flex items-center gap-2 p-3 mb-4 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">
          <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M12 9v4M12 17h.01"/><path d="M3.09 21h17.82a1 1 0 0 0 .86-1.5L13.13 3.4a1 1 0 0 0-1.74 0L2.23 19.5a1 1 0 0 0 .86 1.5z"/></svg>
          {error}
        </div>
      )}
      ...
      {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ STEP 1: BASIC INFO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {step === 0 && (
        <div className="fade-up">
          <div className={SECTION_CLS}>
            <svg className="w-4 h-4 inline mr-1.5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>
            Basic Information
          </div>
          ...
          <div className={SECTION_CLS}>
            <svg className="w-4 h-4 inline mr-1.5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
            Pricing
          </div>
          <div className={gridCols}>
            <div>
              ...
              <span className="text-[10px] text-muted">{pricePaisa ? `à§³ ${(parseInt(pricePaisa) / 100).toFixed(2)}` : ''}</span>
            </div>
            <div>
              ...
              <span className="text-[10px] text-muted">{comparePricePaisa ? `à§³ ${(parseInt(comparePricePaisa) / 100).toFixed(2)}` : ''}</span>
            </div>
            <div className="col-span-full">
          ...
          <div className={SECTION_CLS}>
            <svg className="w-4 h-4 inline mr-1.5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"/></svg>
            SEO
          </div>
          <div className={gridCols}>
            <div className="col-span-full">
          ...
          <div className="flex justify-end mt-6 pt-4 border-t border-line">
            <button onClick={() => { const err = validateBasic(); if (err) { setError(err); return; } setError(''); setStep(1); }} className={`${BTN_CLS} bg-brand text-white hover:opacity-90`}>
              Next: Variants
              <svg className="w-4 h-4 inline ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </button>
          </div>
        </div>
      )}

      {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ STEP 2: VARIANTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {step === 1 && (
        <div className="fade-up">
          <div className={SECTION_CLS}>
            <svg className="w-4 h-4 inline mr-1.5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M7 7h.01M7 3h.01M3 7h.01M3 3h.01M17 7h.01M17 3h.01M13 7h.01M13 3h.01M21 7h.01M21 3h.01"/>
            ...
            <button onClick={addVariant} className="ml-3 inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-semibold rounded-md bg-brand text-white border-0 cursor-pointer hover:opacity-90">
              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5v14"/></svg>
              Add
            </button>
          </div>
                  ...
                  <tr key={i} className="border-t border-line-soft">
                    <td className="px-2.5 py-1.5">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => autoFillSku(i)} title="Auto-fill SKU"
                          className="p-1 border-0 bg-none cursor-pointer text-brand hover:opacity-70 text-xs">â†»</button>
                      </div>
                    </td>
                    <td className="px-2.5 py-1.5">
                    ...
                    <td className="px-2.5 py-1.5">
                      <button onClick={() => removeVariant(i)} disabled={variants.length <= 1}
                        ...
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                      </button>
                    </td>
                  </tr>
                ))}
          ...
          <div className="flex justify-between mt-6 pt-4 border-t border-line">
            <button onClick={() => { setError(''); setStep(0); }} className={`${BTN_CLS} border border-line bg-surface text-ink hover:bg-surface-soft`}>
              <svg className="w-4 h-4 inline mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
              Back
            </button>
      ...
      {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ STEP 3: MEDIA â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {step === 2 && (
        <div className="fade-up">
          <div className={SECTION_CLS}>
            <svg className="w-4 h-4 inline mr-1.5 mb-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M4 16l4.5-4.5a2 2 0 0 1 2.83 0L16 16m-2-2l1.5-1.5a2 2 0 0 1 2.83 0L20 16"/><rect x="2" y="4" width="20" height="16" rx="2"/></svg>
            Media â€” Images will upload after product creation
          </div>
          ...
          >
            <svg className="w-8 h-8 mx-auto text-muted mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path d="M7 16a4 4 0 0 1-.88-7.903A5 5 0 1 1 15.9 6L16 6a5 5 0 0 1 1 9.9M15 13l-3-3m0 0-3 3m3-3v12"/></svg>
            <p className="text-sm text-ink-secondary m-0">Drop images here or click to browse</p>
            <p className="text-[10px] text-muted mt-1 m-0">JPEG, PNG, WebP, GIF, AVIF â€” max 20MB each</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-4">
              {images.map((img, i) => (
                <div key={i} className="relative group rounded-xl overflow-hidden border border-line bg-surface">
                  ...
                  {img.uploaded && <div className="absolute top-1 right-1 w-4 h-4 rounded-full bg-green-500 flex items-center justify-center"><svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M20 6 9 17l-5-5"/></svg></div>}
                  {img.error && <div className="absolute top-1 right-1 w-4 h-4 rounded-full bg-red-500 flex items-center justify-center"><svg className="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M6 18L18 6M6 6l12 12"/></svg></div>}
                </div>
              ))}
            </div>
          )}
      ...
      {/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ STEP 4: REVIEW & PUBLISH â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */}
      {step === 3 && (
        <div className="fade-up">
          <div className="flex items-start gap-2 p-3 mb-5 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800">
            <svg className="w-4 h-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path d="M12 9v4M12 17h.01"/><path d="M3.09 21h17.82a1 1 0 0 0 .86-1.5L13.13 3.4a1 1 0 0 0-1.74 0L2.23 19.5a1 1 0 0 0 .86 1.5z"/></svg>
            <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
            ...
            <div><span className="text-muted text-xs block">Price</span><span className="font-semibold">à§³ {(parseInt(pricePaisa) / 100).toFixed(2)}</span></div>
            <div><span className="text-muted text-xs block">Compare Price</span><span>{comparePricePaisa ? `à§³ ${(parseInt(comparePricePaisa) / 100).toFixed(2)}` : 'None'}</span></div>
          ...
          <div className="overflow-x-auto border border-line rounded-xl text-xs mb-4">
            <table className="w-full border-collapse">
              ...
              <tbody>{variants.map((v, i) => (
                <tr key={i} className="border-t border-line-soft">
                  <td className="px-2.5 py-1.5 font-mono">{v.sku || 'â€”'}</td>
                  <td className="px-2.5 py-1.5">{v.size || 'â€”'}</td>
                  <td className="px-2.5 py-1.5">{v.color || 'â€”'}</td>
                  <td className="px-2.5 py-1.5 text-right">{v.pricePaisa != null ? `à§³ ${(v.pricePaisa / 100).toFixed(2)}` : 'Default'}</td>
          ...
          <div className="flex justify-between mt-6 pt-4 border-t border-line">
            ...
            <button onClick={submit} disabled={submitting}
              ...
              {submitting ? (
                <>
                  <svg className="w-4 h-4 animate-spin" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" className="opacity-30"/><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="3" strokeLinecap="round"/></svg>
                  Creating...
                </>
              ) : (
                <>
