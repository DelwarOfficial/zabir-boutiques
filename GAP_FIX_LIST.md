...
| ID | Gap | Fix |
|---|---|---|
| GAP-P1-001 | Completed. Added a 25-test FraudBD Section 37 suite covering thresholding, time windows, open fallback, half-open recovery, queue retry/timeout behavior, state persistence, and queue-driven approval/cancel flows. | Deepen perf/workerd-specific harness later if infra-level timing proof is needed beyond unit coverage. |
| GAP-P1-002 | Completed for current repo scope. FraudBD external checks now write `api_audit_logs` entries for success, error, timeout, circuit-open fallback, and circuit transitions; 4xx/malformed-body semantics are aligned with breaker rules. | Add extra dashboard/alert integration checks later if monitoring stack becomes part of repo. |
