...
| Repository state assumed | `db/migrations/` head is `0033`; V8 migrations begin at `0034` |
...
| S-10 | P2 | 18.3, `V8_MIGRATION_PLAN.md` 0062 | CSRF reduced to one stateless mechanism; `csrf_nonces` retired; 24-hour dual-key rotation window |
...
|---|---|---|
| `stock_reservations.checkout_id` | Column added | 0034 |
| `idx_stock_reservations_order_active` | **Dropped (retired)** | 0035 |
| `idx_stock_res_order_variant_active` | Partial unique index added on `(order_id, variant_id) WHERE status='active' AND order_id IS NOT NULL` | 0036 |
| `idx_stock_res_checkout_variant_active` | Partial unique index added on `(checkout_id, variant_id) WHERE status='active'` | 0037 |
| `orders.reservation_expires_at` | Column added | 0038 |
| `orders.payment_status` | Column added with CHECK | 0039 |
| `orders.fraud_score` | Column added with CHECK 0–100 | 0040 |
| `orders.fraud_source` | Column added with CHECK | 0041 |
| `orders.created_by_staff_id` | Column added (no FK — see the migration note) | 0042 |
| `orders.staff_override` | Column added with CHECK (0,1) | 0043 |
| `payment_events.provider` | Column added | 0044 |
| `payment_events.provider_event_id` | Column added | 0045 |
| `idx_payment_events_provider_event` | **Unique** index added on `(provider, provider_event_id)` | 0046 |
| `payment_transactions` | Table added | 0047 |
| `coupon_redemptions` | Table added | 0048 |
| `idx_coupon_redemptions_coupon_order` | Unique index added on `(coupon_id, order_id)` | 0049 |
| `tax_rates` | Table added | 0050 |
| `tax_rates` launch row | Seed (`goods`, 0%) | 0051 |
| `cart_activity.cart_version` | Column added | 0052 |
| `courier_shipments` | Table added | 0053 |
| `courier_cod_remittance` | Table added | 0054 |
| `pos_cash_drawer_sessions` | Table added | 0055 |
| `suppliers` | Table added | 0056 |
| `purchase_orders` | Table added | 0057 |
| `goods_receipts` | Table added | 0058 |
| `return_requests.restocked_at` | Column added | 0059 |
| `product_variants.cost_paisa` | Column added with CHECK | 0060 |
| `trg_refund_cap` | Trigger added on `refunds` | 0061 |
| `csrf_nonces` | **Table dropped (retired)** | 0062 |
...
| 0027 `stock_reservations_unique_constraint` | repo `0024` | `0024`, `0027`, and `0028` in the repository all touch this index, and the live index is `idx_stock_reservations_order_active` on `(order_id, variant_id) WHERE status='active'` | **Superseded by 0035 + 0036 + 0037.** The V7 forward SQL (`(order_id)` alone) MUST NOT be applied |
...
| — | — | Repository head `0033_direct_checkout_activity.sql` | V8 migrations begin at **0034** |

Full forward numbering: `0034`–`0062`, one statement each, specified in `V8_MIGRATION_PLAN.md`.
...
Section 6.1 lists it and RT-007 assumes it. It appears in no migration. **Resolution:** migration 0048 creates it and 0049 adds the unique index. This also means no historical redemption data exists to migrate, which is why 0049 is Low risk rather than High.

**NC-03 — `payment_events` has no provider identity.**
F-01 says to add `UNIQUE(provider, provider_event_id)`, but the repository table has neither column. Its existing constraint is `UNIQUE(invoice_id, event_type, status)`, which is not a replay guard: a redelivered event with a different status passes it, and two genuinely distinct events with the same triple would be wrongly rejected. **Resolution:** migrations 0044 and 0045 add the columns before 0046 adds the index, and the index carries a `WHERE provider IS NOT NULL AND provider_event_id IS NOT NULL` predicate so historical NULL rows are excluded explicitly rather than by relying on SQLite's NULL-distinctness — the same subtlety that made the V7 reservation index a no-op.

**NC-04 — the plan's `returns` table is the repository's `return_requests`.**
C-06's fix requires `returns.restocked_at`. **Resolution:** migration 0059 targets `return_requests`, and Section 6.1 notes the naming. The pre-flight asserts the table exists before altering it.
...
| `idx_stock_reservations_order_active` appears nowhere except as a retired artifact | **Pass.** Six remaining occurrences, all of which are the retirement itself: Part-1 §12.3 (retirement statement), Part-2 Guardrail #43, Part-2 §31 checklist ("appears nowhere"), Part-2 §34.4 check #6 ("absent"), Part-2 §35.2 (migration 0035's slug), Part-2 §38 D-23 (detection). Plus this changelog |
...
| `payment_events` has a uniqueness constraint on the provider event id | **Pass.** Migrations 0044–0046, Guardrail #46, check #24 |
| Every migration single-statement and correctly numbered | **Pass.** 0034–0062, one statement each, numbered from the real head of `0033` |
