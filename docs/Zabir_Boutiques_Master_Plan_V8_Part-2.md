...
30. MailChannels-backed `cloudflare_email` is the primary transactional email provider in staging and production. It MUST preserve the adapter contract in Section 17.1 and automatically fall back to the existing Resend adapter on any MailChannels non-2xx response or transport failure. `RESEND_API_KEY` is therefore mandatory anywhere outbound email is enabled. On localhost, using Resend directly is acceptable when MailChannels cannot be exercised from the local runtime; this does not change the deployed primary/fallback design. Manual staff notification is reserved for the case where both providers fail or queue retries are exhausted.
...
- [ ] MailChannels-backed `cloudflare_email` is the primary email provider in staging/production, `EMAIL_PROVIDER=cloudflare_email` is set there, localhost may use Resend directly when needed, and the deployed adapter automatically fails over to Resend without changing caller contracts.
...
| Transactional email providers | Included. MailChannels-backed `cloudflare_email` is the deployed primary, Resend is automatic fallback and practical localhost direct provider, and the email adapter follows the SAME pattern as payments: `src/lib/integrations/email/{provider}/`, interface `sendEmail(request): Promise<SendResponse>`, swapped via `EMAIL_PROVIDER` env var (Section 17.1). |
